...
static void displayIdleScreen(bool init)
{
    ...
    // for TTGO V2's OLED display
    if(OPT_USE_OLED_I2C) {
        uint8_t lq = CRSF::LinkStatistics.uplink_Link_quality;
        int8_t rssi = CRSF::LinkStatistics.uplink_RSSI_1 * -1;
        ...
        uint8_t updated = 0;
        updated |= (last_LQ != lq) ? 1 : 0;
        updated |= (last_rssi != rssi) ? 2 : 0;
        updated |= (last_snr != snr) ? 4 : 0;

        if(updated) {
            display->updateLinkstats(lq, rssi, snr);
            last_LQ = lq;
            last_rssi = rssi;
            last_snr = snr;
        }
