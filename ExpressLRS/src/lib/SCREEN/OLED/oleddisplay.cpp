...
void OLEDDisplay::updateLinkstats(uint8_t lq, uint8_t rssi, uint8_t snr) {
    const static char link_status_icon[] = {'O', '!', 'X'};

    // u8g_font_6x12r
    u8g2->setFont(u8g2_font_profont10_mr);

    char str[25] = {};
    snprintf(str, sizeof(str), "LQ: %c RSSI: %c SNR: %c", link_status_icon[lq], link_status_icon[rssi], link_status_icon[snr]);
    u8g2->drawStr(0, 27, str);
    u8g2->sendBuffer();
}
