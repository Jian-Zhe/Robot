...
// Modified by Jian-Zhe, Su in 2025 to add interface for CrsfSerial
...
serialib::serialib(
    ...
    : Device(Device), Databits(Databits), Parity(Parity), Stopbits(Stopbits)
{
...
/*!
  ...
  */
char serialib::begin(const unsigned int Bauds) {
#if defined (__linux__) || defined(__APPLE__)
...
ssize_t serialib::write(uint8_t byte) {
    return ::write(fd, &byte, 1);
}

ssize_t serialib::write(const uint8_t *buf, size_t len) {
